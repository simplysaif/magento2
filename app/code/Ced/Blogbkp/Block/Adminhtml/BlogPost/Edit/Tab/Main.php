    		...
    		if($val['note']==1)
    			...
    			if($val['note']==1)
    				...
    				if($val['note']==1)
    					...
    					$blog_user_arr = $usercollection->getData();
                        
    					if(isset($blog_user_arr))
    					{
    						$cat = array();
    						foreach($blog_user_arr as $val)
    						{
    							if($val['user_id'] == $user['user_id'])
    							{
    								if(strpos($val['blog_category'],','))
    								{
    									$val['blog_category']= explode(',',$val['blog_category']);
    									foreach($val['blog_category'] as $key=>$value)
    									{
    										$cat[$val['blog_category'][$key]]= $val['blog_category'][$key];
    										$cat1[] = array('label'=>$val['blog_category'][$key], 'value'=>$val['blog_category'][$key]);
    									}        			
    								} else {
    									$cat[$val['blog_category']] = $val['blog_category'];
    									$cat1[] = array('label'=>$val['blog_category'], 'value'=>$val['blog_category']);
    								}
    							}
    						}
    					}
