...
class Miscellaneous extends \Ced\CsMarketplace\Model\Vendor\Rate\Abstractrate
{
     ...
     protected function _getMiscellaneousConditions($vendorId,$storeId) {
     	...
     	$categoryWise = $this->_objectManager->get('\Ced\CsCommission\Helper\Category')->getUnserializedOptions($vendorId);
     	$productTypes =  $this->_objectManager->get('\Ced\CsCommission\Helper\Product')->getUnserializedOptions($vendorId);
     	...
     	return array($productTypes,$categoryWise,$salesCalMethod,$salesRate,$shipCalMethod,$shipRate,$paymentCalMethod,$paymentRate,$servicetaxCalMethod,$servicetaxRate);
          //return array($productTypes,$categoryWise);
     }
     /**
     ...
     public function calculateCommission($grand_total = 0, $base_grand_total = 0, $base_to_global_rate = 1, $commissionSetting = array()) {
     	try {
     		...
     		list($productTypes,$categoryWise,$salesCalMethod,$salesRate,$shipCalMethod,$shipRate,$paymentCalMethod,$paymentRate,$servicetaxCalMethod,$servicetaxRate) = $this->_getMiscellaneousConditions($vendorId,$order->getStoreId());
               //list($productTypes,$categoryWise) = $this->_getMiscellaneousConditions($vendorId,$order->getStoreId());
     		...
     		if(count($productTypes) > 0 || count($categoryWise) > 0) {
     			...
     			//print_r($item->getData('item_id'));die;
     				if($item->getVendorId() && $item->getVendorId() == $vendorId) {
     					...
     					$log = array();
                              //grouped custom my code
                              $quoteItem = $this->_objectManager->create ( '\Magento\Quote\Model\Quote\Item' )->load ( $item->getQuoteItemId() )->getData();
                              $groupedItem=$this->_objectManager->create('\Magento\GroupedProduct\Model\Product\Type\Grouped')->getParentIdsByChild($item->getProductId());
                              foreach ($groupedItem as $key => $pid) {
                                   $parent_id[]=$pid;
                              }
                              
                              if(isset($parent_id)){
                                   foreach ($parent_id as $key => $gid) {
                                        $subgroupedItem=$this->_objectManager->create('\Magento\GroupedProduct\Model\Product\Type\Grouped')->getChildrenIds($gid, true);
                                        foreach ($subgroupedItem as $key => $value) {
                                             if(sizeof($value)>0){
                                                  if(isset($productTypes['grouped'])){
                                                       $product_temp_priority=$productTypes['grouped'];
                                                  }
						...
						$log[$order->getId()][$vendorId]['group'] = $this->_objectManager->get('Ced\CsMarketplace\Model\Vendor')->load($vendorId)->getGroup();
     					...
     					$log[$order->getId()][$vendorId][$item->getQuoteItemId()][$item->getProductId()]['cw']['rate'] = $cw;
                              ...
                              $pt = isset($product_temp_priority['method'])?$this->_objectManager->get('Ced\CsCommission\Helper\Data')->calculateCommissionFee($itemCommission[$item->getQuoteItemId()],$pt,$product_temp_priority['method'], $item->getQtyOrdered()):$this->_objectManager->get('Ced\CsCommission\Helper\Data')->calculateCommissionFee($itemCommission[$item->getQuoteItemId()],$pt,$this->_scopeConfig->getConfig('ced_vpayments/general/commission_mode_default'), $item->getQtyOrdered());
                              $cw = isset($category_temp_priority['method'])?$this->_objectManager->get('Ced\CsCommission\Helper\Data')->calculateCommissionFee($itemCommission[$item->getQuoteItemId()],$cw,$category_temp_priority['method'], $item->getQtyOrdered()):$this->_objectManager->get('Ced\CsCommission\Helper\Data')->calculateCommissionFee($itemCommission[$item->getQuoteItemId()],$cw,$this->_scopeConfig->getConfig('ced_vpayments/general/commission_mode_default'), $item->getQtyOrdered());
     					...
     					$cf = $this->_scopeConfig->getConfig('ced_vpayments/general/commission_fn', \Magento\Store\Model\ScopeInterface::SCOPE_STORE,$order->getStoreId());
     					...
     					/* END */
                              ...
                              if(isset($quoteItem['parent_item_id']) && $quoteItem['parent_item_id']!=""){

                              }elseif(isset($productTypes['grouped'])){
                                   if($gpd==0){
