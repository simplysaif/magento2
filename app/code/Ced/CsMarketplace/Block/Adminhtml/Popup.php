...
class Popup extends \Magento\Backend\Block\Widget\Container
{
    ...
    public function getModules(){
        $modules = $this->_objectManager->get('\Ced\CsMarketplace\Helper\Feed')->getCedCommerceExtensions();
        ...
        foreach ($modules as $moduleName=>$releaseVersion)
        {
            ...
            if(is_array($h) && isset($h['domain']) && isset($h['module_name']) && isset($h['license']) && $h['module_name'] == $m && $h['license'] == $helper->getStoreConfig(\Ced\CsMarketplace\Block\Extensions::HASH_PATH_PREFIX.$m)){}else{ 
                $args .= $m.',';
            }   
        }
