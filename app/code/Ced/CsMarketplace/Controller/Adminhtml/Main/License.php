...
class License extends \Magento\Backend\App\Action
{
            ...
            if ($response && isset($response['hash']) && isset($response['level'])) {
                ...
                try {
                    ...
                    if (is_array($valid) &&
                        ...
                        $path = \Ced\CsMarketplace\Block\Extensions::HASH_PATH_PREFIX . strtolower($postData['module_name']) . '_hash';
                        $this->_feedHelper->setDefaultStoreConfig($path, $response['hash'], 0);
                        $path = \Ced\CsMarketplace\Block\Extensions::HASH_PATH_PREFIX . strtolower($postData['module_name']) . '_level';
                        $this->_feedHelper->setDefaultStoreConfig($path, $response['level'], 0);
                        $json['success'] = 1;
                        $json['message'] = __('Module Activated successfully.');
                        $details = $this->_objectManager->get('Ced\CsMarketplace\Block\Adminhtml\Extensions\Details');
                        $details->checkLicense();
    ...
    private function getLicenseActivateUrl()
    {
        if (is_null($this->_licenseActivateUrl)) {
            $this->_licenseActivateUrl = ($this->_feedHelper->getStoreConfig(\Ced\CsMarketplace\Block\Extensions::LICENSE_USE_HTTPS_PATH) ? 'https://' : 'https://')
