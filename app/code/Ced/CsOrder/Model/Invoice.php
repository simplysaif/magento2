...
class Invoice extends \Ced\CsMarketplace\Model\FlatAbstractModel
{
    ...
    public function updateTotal($invoice, $view = true)
    {
        ...
        $helperData = $this->_objectManager->create('Ced\CsOrder\Helper\Data');
        if (!is_object($vorder)) {
            return $invoice; 
        }
    ...
    {
        $vorder =  $this->_objectManager->create('Ced\CsMarketplace\Model\Vorders')->getVorderByInvoice($invoice);
        $helperData = $this->_objectManager->create('Ced\CsOrder\Helper\Data');
        if (!is_object($vorder)) {
            return $invoice; 
        }
