...
class Invoice extends \Magento\Sales\Model\Order\Pdf\Invoice
{
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        $vendorId = $objectManager->get('Magento\Customer\Model\Session')->getVendorId();
        $public_name = $objectManager->get('Magento\Customer\Model\Session')->getVendor();
    ...
    /*Custome Code to Add Seller Information START*/
        ...
        $page->drawText(__("Sold By :  ".$public_name['public_name']), 35, $this->y+20, 'UTF-8');
        ...
        $y=30;
        $count=0;
        //$vattr = array('name','public_name','shop_url');
            ...
            foreach (str_split($value_code, 45) as $_value) {
                ...
                /*if (in_array($value_code, $vattr)) {
                    $page->drawText(strip_tags(trim($value['frontend_label'])), $paymentLeft, $top -=$y, 'UTF-8');
                        ...
                        $yPayments -= 15;
                }else*/if ($value['use_in_invoice']=='1' && $vendor_info->getData($_value)) {
                    $count++;
                    $value_code = preg_replace('/<br[^>]*>/i', "\n", $value['attribute_code']);
                    foreach (str_split($value_code, 45) as $_value) {
                        /*if($value_code=='public_name'|| $value_code=='name'|| $value_code=='shop_url'){continue;}*/
                        $page->drawText(strip_tags(trim($value['frontend_label'])), $paymentLeft, $top -=$y, 'UTF-8');
                        $page->drawText(strip_tags(trim($vendor_info->getData($_value))), $paymentLeft1,$top -=0, 'UTF-8');
                        $yPayments -= 15;
                    }
