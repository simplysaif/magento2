 ...
 *
 * @category    Ced
 * @package     Ced_MobilLogin
 * @author 		CedCommerce Core Team <connect@cedcommerce.com>
 * @copyright   Copyright CedCommerce (http://cedcommerce.com/)
 * @license      http://cedcommerce.com/license-agreement.txt
 */ 
 
namespace Ced\MobilLogin\Controller\Adminhtml\Main;
...
class License extends \Magento\Backend\App\Action
{
    ...
    public function __construct(
        ...
        $this->_feedHelper = $this->_objectManager->get('Ced\MobilLogin\Helper\Feed');
    }
        ...
        if($postData){
            ...
            if ($response && isset($response['hash']) && isset($response['level'])) {
                ...
                try {
                    ...
                    {
                        $path = \Ced\MobilLogin\Block\Extensions::HASH_PATH_PREFIX.strtolower($postData['module_name']).'_hash';
                        $this->_feedHelper->setDefaultStoreConfig($path, $response['hash'], 0);
                        $path = \Ced\MobilLogin\Block\Extensions::HASH_PATH_PREFIX.strtolower($postData['module_name']).'_level';
    ...
    private function getLicenseActivateUrl()
    {
        if (is_null($this->_licenseActivateUrl)) {
            $this->_licenseActivateUrl = ($this->_feedHelper->getStoreConfig(\Ced\MobilLogin\Block\Extensions::LICENSE_USE_HTTPS_PATH) ? 'https://' : 'http://')
