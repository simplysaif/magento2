...
class BeforeLoadLayout  implements ObserverInterface
{
    ...
    public function __construct(
        \Ced\MobileLogin\Model\Feed $_feed,
        ...
        $this->_feedHelper = $this->_objectManager->get('Ced\MobileLogin\Helper\Feed');
    }
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
       try {
            ...
            if($controllerModule !== 'magento_config' || $request->getParam('section') !== 'ced_mobilelogin'){ return $this; } 
            $helper = $this->_objectManager->create('Ced\MobileLogin\Helper\Feed');
            ...
            foreach ($modules as $moduleName=>$releaseVersion)
            {
                ...
                $h = $this->_objectManager->create('Ced\MobileLogin\Helper\Data')->getStoreConfig(\Ced\MobileLogin\Block\Extensions::HASH_PATH_PREFIX.$m.'_hash'); 
         
                for($i=1;$i<=(int)$this->_objectManager->create('Ced\MobileLogin\Helper\Data')->getStoreConfig(\Ced\MobileLogin\Block\Extensions::HASH_PATH_PREFIX.$m.'_level');$i++)
                {  
                        $h = base64_decode($h);
                }

                $h = json_decode($h,true);            
                if(is_array($h) && isset($h['domain']) && isset($h['module_name']) && isset($h['license']) && strtolower($h['module_name']) == $m && $h['license'] == $this->_objectManager->create('Ced\MobileLogin\Helper\Data')->getStoreConfig(\Ced\MobileLogin\Block\Extensions::HASH_PATH_PREFIX.$m) && $this->_objectManager->create('Ced\MobileLogin\Helper\Data')->getStoreConfig(\Ced\MobileLogin\Block\Extensions::HASH_PATH_PREFIX.$m)!=null)
                {  
                }else{
            ...
            foreach ($result as $moduleName => $response) {
                if ($response && isset($response['hash']) && isset($response['level'])) {
                    ...
                    try {
                        ...
                        {
                            $path = \Ced\MobileLogin\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName).'_hash';
                              $this->_feedHelper->setDefaultStoreConfig($path, $response['hash'], 0);
                            $path = \Ced\MobileLogin\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName).'_level';
                             $this->_feedHelper->setDefaultStoreConfig($path, $response['level'], 0);
                            $path = \Ced\MobileLogin\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName);
    ...
    private function getLicenseActivateUrl()
    {
        if (is_null($this->_licenseActivateUrl)) {
            $this->_licenseActivateUrl = ($this->_feedHelper->getStoreConfig(\Ced\MobileLogin\Block\Extensions::LICENSE_USE_HTTPS_PATH) ? 'https://' : 'http://')
