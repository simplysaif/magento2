...
class DeleteValue extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
        ...
        $data = $this->_getSession->getData('quoteData');
        if(count($data)==0){
            $this->messageManager->addSuccess ( __ ( 'Item was deleted successfully.You have no item in your quote cart.' ) );
            return $this->_redirect('customer/account/login');
        }
        else{
            $this->messageManager->addSuccess ( __ ( 'Item was deleted successfully' ) );
