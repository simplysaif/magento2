 ...
 * CedCommerce
  ...
  */
namespace Ced\TeamMember\Block\Member;
...
class Conversation extends \Magento\Framework\View\Element\Template
{
  ...
  public function getTeamMemberConversation()
  {
  ...
  {  
    $token_s=$this->session->getTeamMemberDataAsLoggedIn()->getEmail()."-".$this->getRequest()->getParam('vendoremail');
    return $token_s;
  }

   public function getRecevierToken()
  {  
    $token_r = $this->getRequest()->getParam('vendoremail')."-".$this->session->getTeamMemberDataAsLoggedIn()->getEmail();
    return $token_r;
  }
