	...
	public function execute() {
		...
		if ($data) {
			...
			if ($result) {
				...
				$model->setData ( 'badge_description', $data ['badge_description'] );
				if ($result ["file"] != "") {
					$model->setData ( 'badge_image', $result ["file"] );
				}
				...
				$model->setData ( 'sales', $data ['sales'] );
			}
			try {
				$model->save ();
				...
				if ($this->getRequest ()->getParam ( 'back' )) {
					return $resultRedirect->setPath ( '*/badges/edit', [ 
							'grid_record_id' => $model->getId (),
							'_current' => true 
					] );
				}
				return $resultRedirect->setPath ( 'csvendorbadges/badges/badgesview' );
