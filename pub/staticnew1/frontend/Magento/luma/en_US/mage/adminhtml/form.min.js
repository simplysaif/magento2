...
catch(e){}};window.FormElementDependenceController=Class.create();FormElementDependenceController.prototype={initialize:function(elementsMap,config){var idTo,idFrom,values,fromId,radioFrom;if(config){this._config=config;}
...
if(shouldShowUp){currentConfig=this._config;if(inputs){inputs.each(function(item){if((!item.type||item.type!='hidden')&&!($(item.id+'_inherit')&&$(item.id+'_inherit').checked)&&!(currentConfig['can_edit_price']!=undefined&&!currentConfig['can_edit_price'])){item.disabled=false;jQuery(item).removeClass('ignore-validate');}});}
