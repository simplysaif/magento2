...
class SqlBuilder
{
    ...
    private function addScopedAttributeConditions(
        ...
        $entityFieldName =
            $this->indexResource->getConnection()->tableColumnExists($table, 'entity_id') ? 'entity_id' : 'row_id';
        $select->from(
            ['attr_d' => $table],
            'COUNT(*)'
        )->joinLeft(
            ...
            sprintf('attr_d.%s = e.entity_id', $entityFieldName)
        )->where(
            'attr_d.attribute_id=?',
            $attribute->getId()
        )->where(
